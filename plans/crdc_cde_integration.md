...
**Conclusion at the time**: no native-Synapse path found that replaces the CSV→JSON-LD
generation step. **This was wrong — corrected same day, see below.** `synapseclient.services.json_schema`
was the wrong module to check; the real one, `synapseclient.extensions.curator`, wasn't found
because it doesn't exist in `synapseclient` 4.8.0 (the version installed at the time) — only in
4.13.0+.

### `make qc`/schematicpy (#17) — actually resolved: `synapseclient.extensions.curator`

The user pointed at the specific module by name. Upgrading the test environment's
`synapseclient` from 4.8.0 → 4.13.0 (hitting and fixing an unrelated `pkg_resources`/`setuptools`
import break along the way — recent `setuptools` no longer vendors `pkg_resources` by default,
needed `setuptools<81`) surfaced it: `synapseclient.extensions.curator.generate_jsonld` and
`.generate_jsonschema`, a genuine, complete, actively-maintained replacement for both
`schematic schema convert` and `create_json_from_model.py`'s generation step. No stability
warnings on this module (unlike the `services.json_schema` beta warning).

**Confirmed working directly against this repo's real `mc2.model.csv`** (605 attributes, no
login/credentials required for local generation):
- `generate_jsonld` produced a complete, valid `mc2.model.jsonld` (4.1MB) in one call.
- `generate_jsonschema` produced all 32 schema files matching the Makefile's `DATA` list exactly,
  after fixing **one real, genuine validation error** it caught in our own data:
  `Biospecimen Taxonomy ID` had `columnType=number` together with a string `Pattern`
  (`^\d+$`) — an inconsistent combination this validator correctly rejects (a numeric-looking
  value checked by a string pattern should be typed `string`). Fixed; re-verified this was the
  *only* such issue anywhere in the model (scripted an iterate-fix-retry loop against a scratch
  copy — one fix, then everything generated cleanly).
- `create_json_from_model.py` (the existing `generate-json` target) was already using this same
  module — it had already been migrated at some point before this session; only `make convert`
  was still on the old, broken `schematic` CLI.

**Implemented**: added `convert_model_to_jsonld.py` (mirrors `create_json_from_model.py`'s
existing style), repointed `make convert` at it, regenerated `mc2.model.jsonld` and all 32
`json_schemas/*.json` files from the current (previously very stale — predating this entire
session) model. Two schema files removed as genuinely stale leftovers with pre-rename names no
longer in the Makefile's `DATA` list (`ImagingChannel.json`, `VisiumRNALevel1-4.json`/
`VisiumAuxiliaryFiles.json` — all superseded by their correctly `10x`-prefixed current
equivalents, which are included).

**Also fixed, not just the code**: `requirements.txt` pinned both `synapseclient>=4.8.0` *and*
`schematicpy>=25.8.1` — but `schematicpy==25.8.1` itself requires `synapseclient==4.8.0` exactly,
directly conflicting with the `synapseclient>=4.13.0` this fix needs. Removed `schematicpy`
from `requirements.txt` entirely (it's deprecated, was already non-functional for its one
remaining use here, and actively conflicted with the working toolchain) and bumped
`synapseclient` to `>=4.13.0`. Updated `CLAUDE.md`'s repo description and `make convert` command
line accordingly.

**Not resolved — flagged as a real follow-up**: `make qc`/`qc_convert` still calls the old
`schematic schema convert` against `qc_model/mc2_qc.model.csv`, a separately hand-formatted copy
of the model carrying schematicpy's older required columns (`Parent`, `DependsOn Component`).
This mechanism predates this session, isn't regenerated by anything in the Makefile, and its
exact purpose relative to the main model isn't fully clear. Since the new toolchain doesn't need
those extra columns at all, this entire mechanism may be obsolete — but retiring or fixing it is
a repo-structure decision this session didn't make unilaterally. `make qc`'s `collate` half still
works fine; only `qc_convert` is unresolved.

## Round 10 — propagate all of the above into `kg-pipeline/`

`kg-pipeline/` is a separate LinkML/RDF knowledge-graph build (Stage 0: `make mc2-model-linkml`
converts `mc2.model.csv` + `modules/mapping.yaml` into `schema/mc2_model.linkml.yaml`, imported by
the hand-authored Stage 1 `schema/cckp_portal.linkml.yaml`, which resolves real CCKP data against
MC2 controlled vocabularies via `scripts/harmonize.py` and mints RDF via `scripts/build_triples.py`
— full architecture in `kg-pipeline/README.md`). Every module round above (1–9) needed propagating
into this second pipeline, since it references MC2 attribute names by string in several places
that aren't touched by `make collate`. Live Synapse extraction (`make extract`) was not available
in this environment — worked from the `data/raw/*.csv` already cached from a prior real extract,
via `make harmonize` / `make triples` / `make validate`.

### What was found and fixed

- **`scripts/extract_mc2_assay_metadata.py`**: `ANNOTATION_KEY_TO_ATTRIBUTE` still pointed at the
  pre-consolidation names (`File Assay`, `File Species`, `File Tissue`, `File Tumor Type`, `File
  Data Use Codes`) retired in the round-2/round-7 consolidations. Repointed to `Assay`, `Species`,
  `Tissue`, `Tumor Type`, `Data Use Codes`. Also confirmed, via `SchemaView.induced_class("File
  View").attributes`, that `Tissue`/`Tumor Type` are now genuinely attached to the `File View`
  class post-consolidation — resolving a gap the script's own docstring had flagged as outstanding
  — and updated that docstring accordingly, noting that `link_sagebrain.py`'s separate manual
  harmonization pass for those two fields may now be redundant, though this hasn't been verified
  end-to-end against live-extracted data.
- **`scripts/link_sagebrain.py`**: same renames (`File Tissue`/`File Tumor Type`/`File Species` →
  bare names) across 8 code/docstring occurrences; one docstring had already been partially updated
  to describe the fix while the code underneath still used the old literal strings — brought fully
  into sync.
- **`schema/cckp_portal.linkml.yaml`**: 13 stale `mc2_enum:` annotations repointed to the enum
  names that survived consolidation (Dataset/File/Publication Assay|Species|Tissue|Tumor Type →
  the shared `Assay|Species|Tissue|Tumor Type Enum`; Tool/Resource License → `License Enum`;
  Dataset/Publication/Tool/Resource Grant Number → `DSP Dataset Grant Number Enum`, the CV that
  survived the round-4 Grant Number/Grant Affiliation consolidation). `Grant.consortium`'s
  `mc2_enum` annotation was **removed entirely**, not repointed: its old target (`Grant Consortium
  Name Enum`, backed by Consortium Affiliation) was fully retired this session in favor of
  Consortium Key, whose real value space (`program.ccbir`, `program.csbc`, ...) is a different,
  machine-slug format that doesn't match what live CCKP `Grant.consortium` data actually stores —
  no surviving MC2 attribute carries the bare-acronym form (`CCBIR`, `CSBC`, ...), so there is
  nothing correct to repoint to. Left as an explicitly documented gap (inline YAML comment) rather
  than guessing. `mappings/crosswalks/consortium_to_scdm_program.tsv` already contains the exact
  bare-acronym→`program.X` crosswalk that could resolve this — noted as a pointer for whoever makes
  that follow-up call, not applied here.
- **SSSOM crosswalks** (`mappings/sssom/`): `scripts/harmonize.py::write_sssom` derives each
  filename from its enum name, so the same consolidations that merged MC2 attributes also merge
  their SSSOM files on a real harmonize run. Confirmed via an actual `make harmonize` run (not
  guessed) and deleted 12 now-orphaned per-entity files (`dataset_assay`, `file_assay`,
  `publication_assay`, `dataset_species`, `file_species`, `dataset_tissue`, `publication_tissue`,
  `dataset_tumor_type`, `publication_tumor_type`, `resource_license`, `tool_license`,
  `grant_consortium_name`), added 3 consolidated replacements (`assay.sssom.tsv` — 367 resolved
  terms, `tissue.sssom.tsv` — 114, `tumor_type.sssom.tsv` — 180), and refreshed `license.sssom.tsv`
  and `species.sssom.tsv` in place (both were already the correct consolidated file, just missing
  newly-in-scope terms, e.g. `species.sssom.tsv` gained "African Bush Elephant"). Verified two
  files as correctly *unaffected*: `dataset_file_formats.sssom.tsv` regenerated byte-identical
  (File Format vs. Dataset File Formats correctly stayed separate) and `tool_entity_type.sssom.tsv`
  likewise (Tool Entity Role's CDE-tag-only change had no crosswalk impact). `file_format.sssom.tsv`
  belongs to the separate, gitignored, credential-gated MC2-assay-metadata pipeline and could not
  be re-run live in this environment — checked statically instead (its source attribute, "File
  Format", was not renamed by any round this session).
- **Tests**: fixed 5 failures across `test_mc2_assay_file_view.py`, `test_schema_loads.py`,
  `test_harmonize.py`, and 2 in `test_link_sagebrain.py` (straightforward renames matching the
  script/schema fixes above), plus rewrote `test_build_triples_grant.py`'s
  `test_confirmed_unmappable_value_gets_provisional_term_not_bare_literal` — see the open question
  below. `make test`: **73/73 passed**.
- **Regenerated**: `schema/mc2_model.linkml.yaml` + `.ttl` + conversion/prefixes reports (`make
  mc2-model-linkml`), `schema/cckp_portal.ttl` (`make schema`), `mappings/coverage_baseline.json`
  (`make update-coverage-baseline`: `Dataset.assay` 64→63, `Publication.assay` 359→355 — real
  improvements from the merged Assay CV; `Grant.consortium` (160) removed entirely, no longer
  tracked since it has no `mc2_enum`; 37 fields tracked, was 38).

### Open design question, flagged not resolved: `Grant.consortium` and `confirmed_unmappable.tsv`

`scripts/build_triples.py` gates its entire per-field Term-edge block — including the
`confirmed_unmappable.tsv`-driven provisional-IRI fallback (`cckp:provisional true`) — on
`meta["mc2_enum"]` being truthy. Removing `Grant.consortium`'s `mc2_enum` annotation (necessary,
above) also silently disabled that provisional-IRI mechanism for it: `"CCBIR"` is a confirmed-
unmappable entry (an NCI-internal program acronym with no real ontology/registry home), and used to
get an addressable, explicitly-flagged provisional term edge; it now gets only the bare literal,
identical to any other unresolved string. The `confirmed_unmappable` mechanism exists precisely for
values a human has confirmed will never resolve to a real CV — that doesn't logically require the
field to have ever had one, so gating it on `mc2_enum` may be too strict. Two ways to resolve, not
chosen here: (a) decouple the provisional-IRI path in `build_triples.py` from requiring
`mc2_enum`, restoring provisional-term treatment for `Grant.consortium`'s acronyms, or (b) accept
this as correct given the field's CV is genuinely gone. `test_build_triples_grant.py` was updated
to assert the current (b) behavior explicitly, with the tension documented inline — left for a
deliberate decision alongside the broader Consortium CV gap.

### Follow-up already flagged, restated here for tracking: `CRDC_CDE:` unregistered prefix

`make mc2-model-linkml`'s conversion report (`schema/mc2_model.linkml_conversion_report.md`) has
carried, since the CDE integration rounds began adding `CRDC_CDE:<id>` tags to the `Properties`
column, one warning: `CRDC_CDE:` is an unregistered CURIE prefix, so a placeholder namespace was
written. Not urgent, not blocking `make validate` or any test — but it needs a real URI registered
in `schema/mc2_model.linkml.yaml`'s (or its generating script's) `prefixes:` block at some point.

### Verification

`make test`: 73/73 passed. `make validate`: coverage gate shows **no regressions** vs. the updated
baseline; SHACL validation **conforms** (296,190 triples checked). Everything above was verified
against a real `make harmonize`/`make triples`/`make validate` cycle using the cached
`data/raw/*.csv` extract already present in this environment — nothing here relied on live Synapse
credentials, and nothing was fabricated where real data wasn't available (the `Grant.consortium`
gap and the MC2-assay-metadata pipeline's `file_format.sssom.tsv` are both explicitly flagged as
unverified-live rather than assumed fine).

## Round 11 — the three round-10 follow-ups, verified and resolved

The coordinator did preliminary legwork on all three round-10 open items and proposed concrete
fixes. Each was verified against real cached data (not taken on assertion) before implementing —
one proposal held up, one didn't (a genuinely better fix was found and used instead), and one
already-suspected-safe assumption was confirmed true via an actual end-to-end trace.

### 1. `Grant.consortium` (and Dataset/Publication/Tool's own `consortium` fields): repoint rejected

Proposed fix: repoint `mc2_enum` to the surviving `Consortium_id Enum` (`program.ccbir`,
`program.htan`, ... — 11 real entries derived from `modules/consortium/consortium_id.csv`).

**Checked against real cached data first, per instruction.** `data/raw/{Dataset,Publication,
Tool,Grant}.csv`'s `consortium` columns all store bare acronyms (`CCBIR`, `CSBC`, `HTAN`, `ICBP`,
`MetNet`, `NCI`, `PDMC`, `PS-ON`, `Sage Bionetworks`, `TEC`, and multivalued combinations like
`CCBIR|MetNet`) — never `program.X` format. That alone would make a direct repoint useless without
an alias layer. But a second, more fundamental problem makes the repoint actively wrong, not just
incomplete: `harmonize.py`'s `load_cv_lookup` only indexes a CV source row if its `Ontology
Identifier` column is non-empty (and CURIE- or URL-shaped) — and every row in `consortium_id.csv`
has an **entirely empty** `Ontology Identifier` column (it's an internal Sage program-ID registry,
not an ontology-backed CV). Repointing to `Consortium_id Enum` would therefore build a permanently
**empty** lookup dict, meaning every consortium value would be reported unmapped — the field would
look CV-backed while silently resolving nothing, strictly worse than the current honest
"untracked, no mc2_enum" state. Not implemented.

**What actually resolves this gap, already built**: `mappings/crosswalks/consortium_to_scdm_program.tsv`
(bare acronym → `program.X`, generated by `scripts/crosswalk_scdm.py`) + `scripts/link_scdm.py`,
which mints a `sagecdm:Program` node and a `cckp:consortiumRef` edge per resolved value — but only
for crosswalk rows with `reviewed=true`. Checked the actual file: **every row is currently
`reviewed=false`** (auto-generated, `scdm_status`/`scdm_funding_source`/etc. blank, awaiting human
curation per `crosswalk_scdm.py`'s own docstring). So this isn't a missing mechanism at all — it's
an existing, deliberate, already-tested one (`test_crosswalk_scdm.py`, `test_link_scdm.py` both
pass) gated behind a review step nobody has done yet. That review — not a schema change — is what
would actually turn consortium linkage on.

Documented the full finding as an inline comment on `Grant.consortium` in `cckp_portal.linkml.yaml`
(the fullest writeup, since it's the field the earlier round's tests/comments already focused on),
plus a short one-line pointer to it on Dataset/Publication/Tool's `consortium` slots (they never
had an `mc2_enum` to begin with, so there was no existing inaccuracy to fix there — just added for
discoverability). Comment-only change: no regeneration needed, no test or RDF-output changes.

### 2. `CRDC_CDE:` prefix: the proposed URL doesn't actually resolve — registered a verified one instead

Proposed fix: register `CRDC_CDE: https://cadsr.cancer.gov/onedata/dmdirect/NIH/NCI/CO/CDEDD?filter=CDEDD.ITEM_ID=`
(derived from the classic caDSR UI's "Deep Link" field, version qualifier dropped since a CURIE
prefix has only one variable slot).

**Verified before registering, per explicit instruction** — and it failed: a plain `curl` GET
against this URL (tested with and without the `ver_nr` qualifier, with and without a cookie jar and
a real browser user-agent) redirects every time to `https://cadsr.cancer.gov/onedata/Home.jsp`, a
generic anonymous landing page — never a real CDE record. This is a classic JSP-portal URL that
depends on session/JS state a stateless request can't establish; it's not actually a resolvable,
bookmarkable CURIE target despite being labeled "Deep Link."

**Registered the real, working alternative instead**: the caDSR REST endpoint this session already
found and used for live permissible-value fetching in Round 8,
`https://cadsrapi.cancer.gov/rad/NCIAPI.v1_0:NciApiRad/DataElement/`. Verified directly against two
real CDE IDs from the model (`88` and `12445832`) — both return real `DataElement` records (the
second correctly returning "Specimen Material Category," matching Round 9's re-scope). Wired via
`csv_to_linkml.py`'s existing `--prefix PREFIX=URI` mechanism, added to `kg-pipeline/Makefile`'s
`mc2-model-linkml` target. `make mc2-model-linkml`: **0 warnings** (was 1). Verified CURIE
multiplicity still works after regeneration (an attribute with two `CRDC_CDE:` tags still gets both
under `exact_mappings`). `make schema` regenerated both `.ttl` files — large diffs are owlgen's
known non-deterministic restriction ordering (confirmed via `git diff`, content is otherwise
unchanged apart from the new `CRDC_CDE` prefix IRI itself). The pre-existing, unrelated `SPDX`
prefix gap flagged by `resolve_prefixes.py` is untouched (`mc2_model_prefixes_report.md` has zero
diff), and `CDE:` (the original, non-CRDC caDSR-CDE tag) is unaffected — it was already handled by
a separate, deliberate special case in `csv_to_linkml.py` (stored as a bare `cde_id` annotation,
not a registered CURIE at all) that predates this round and wasn't touched.

### 3. `link_sagebrain.py`'s manual Tissue/Tumor Type pass: confirmed genuinely distinct, not redundant

Question: now that `Tissue`/`Tumor Type` resolve via the standard `harmonize.py` class-driven pass
(round 10's consolidation), does `link_sagebrain.py`'s separate manual pass just redo that work?

**Traced end-to-end against real cached data, not inferred from code alone.** First confirmed the
literal duplication is already gone: `link_sagebrain.py` no longer calls `load_cv_lookup` anywhere
(grepped) — that was the actual redundant part, and round 10 already removed it (its own docstring
says so). What's left is not redundant: it reads the `{field}_ontology_iri` values `harmonize.py`'s
File View pass already resolved (real NCIT terms) and cross-walks *those* further to sagebrain-
model's own anchor ontologies — UBERON for tissue, MONDO for tumor type — via the `high`-confidence
rows of `mappings/crosswalks/*.sssom.tsv`, then emits sagebrain-specific `source_tissue`/
`has_pathology` predicates on a synthetic `biolink:MaterialSample` node per `Biospecimen Key`. That
target ontology, that predicate vocabulary, and that node shape don't exist anywhere in the
standard `harmonize.py`/`build_triples.py` output — a genuine, purpose-built federation step
consuming the standard mechanism's output, not duplicating its computation.

Couldn't prove this with a live fresh extract (no Synapse credentials here): the cached
`data/mc2_assay/raw/File View.csv` predates this session's renames, so its column headers still
read `File Tissue`/`File Assay` — re-running `make harmonize-mc2-assay` against it as-is resolves
nothing for these fields (a staleness artifact, not a mechanism failure). Verified the mechanism
properly instead by renaming that same real cached file's headers to the names the fix now expects
(same real values, e.g. "Breast"/"Triple-Negative Breast Carcinoma," just current header names) and
re-running the full chain in an isolated scratch copy: `harmonize.py` resolved 3 distinct `Tissue`
terms and 4 distinct `Tumor Type` terms to real NCIT IRIs; `link_sagebrain.py` then emitted 10 real
UBERON/MONDO triples across 4 `MaterialSample` nodes from them. Both halves confirmed working and
genuinely distinct. Updated `extract_mc2_assay_metadata.py`'s docstring from a hedge ("may now be
redundant... hasn't been verified") to this confirmed conclusion.
